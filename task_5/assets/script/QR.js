$(document).ready(function () {
  ...
  $("#addMoreBtn").click(function () {
    $(".addMoreSec").toggleClass("active");
    $(this).hide();
    $("#closeaddMore").show();
  });

  $("#closeaddMore").click(function () {
    $(".addMoreSec").toggleClass("active");
    $(this).hide();
    $("#addMoreBtn").show();
  });
  ...
  $("#QRgenerator_form").on("submit", (e) => {
    ...
    if (!vpa || !vpa.match(/^([^\s@]+)@([^\s@]+)$/)) {
      ...
      setTimeout(() => {
        $("#vpaError").text(
          "Invalid VPA format (e.g., name/number@bankname)!!!"
        );
        $(".fa-spin-pulse").hide();
        $("#vpaInput").focus();
      }, 500);
    ...
    if (isValid) {
      ...
      setTimeout(() => {
        ...
        //  const bankName = extractBankNameFromVPA(vpa);

        //  if (!bankNames.includes(bankName)) {
        //    bankNames.push(bankName);
        //    console.log("New bank name added:", bankName);
        //  }
        $(".fa-spin-pulse").hide();
...
function renderUserList() {
  ...
  if (!users.length) {
    ...
    userListModal.html(`<h1 style="color:gray">No users found!</h1>`);
  }
